...
export default {
  ...
  async beforeRouteEnter(to, from, next) {
    // const docSnapshot = songsCollection.doc(to.params.id).get();
  ...
  computed: {
    ...
    sortedComments() {
      return this.comments.slice().sort((a, b) => {
        if (this.sort === '1') {
          return new Date(b.datePosted) - Date(a.datePosted);
        }
        return new Date(a.datePosted) - Date(b.datePosted);
      });
    },
  },
  methods: {
